...
public static class Program
{
    ...
    // start. Move Overhead is the only one this engine actually acts on
    // (folded into TimeManagement's budget); the rest are accepted and
    // otherwise ignored (no transposition table/threading/tablebases/WDL
    // model exist yet to hook them up to).
    ...
    private static void HandleSetOption(string command)
    {
        ...
        // Threads/Hash/SyzygyPath/UCI_ShowWDL: recognized (see the "uci"
        // handler's option list) so lichess-bot's setoption doesn't error,
        // but otherwise unused — see the field comment on _moveOverheadMs.
    }
    ...
    private static void HandleGo(string command)
    {
        ...
        var (board, positionCounts) = EngineInterop.ReplayHistory(_startFen, _movesHistory);

        // The UCI protocol never sends a new "go" before the previous one
        // answered with bestmove (itself only ever preceded by "stop"), but
        // wait defensively rather than racing two searches over the same
        // Board/positionCounts.
        _activeSearch?.Wait();
        ...
        if (depth is not null && movetimeMs is null && !infinite && !hasClock)
        {
            ...
            {
                var result = Search.Run(board, new SearchOptions { Depth = depth }, int.MinValue + 1, int.MaxValue, positionCounts, 0);
        ...
        _activeSearch = Task.Run(() =>
        {
            var result = Search.RunIterative(board, new SearchOptions { MovetimeMs = (int)System.Math.Min(budgetMs, int.MaxValue) }, positionCounts, deadline);
            EmitBestMove(board, result);
        });
    }
